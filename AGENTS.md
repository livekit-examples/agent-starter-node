...
For turn-level checks that don't need a live session, use the in-process [testing framework](https://docs.livekit.io/agents/start/testing/). `src/agent.test.ts` has a commented-out example. Run those tests with `pnpm test`.
