...
For turn-level checks that don't need a live session, the LiveKit Agents [unit testing framework](https://docs.livekit.io/testing/unit-tests/) runs your agent in-process under `vitest`. [`src/agent.test.ts`](src/agent.test.ts) has a commented-out example.
